...
export function ReservationList({ reservations, onEdit, onDelete, userRole = "student" }) {
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      min={1}
                      max={userRole === "guest" ? 3 : 6}
                      value={editForm.group_size}
                      onChange={(e) => setEditForm({ ...editForm, group_size: Number(e.target.value) })}
              ...
              <div>
                <div className="flex justify-between items-start mb-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-gray-700">
                      ...
                      <span className="text-sm font-bold">{formatDateTime(reservation.booking_time)}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-500">
                      <User className="size-3" />
                      <span className="text-xs capitalize">{reservation.user_role}</span>
                      <span className="text-gray-300">|</span>
                      <Users className="size-3" />
                      <span className="text-xs">{reservation.group_size} People</span>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-1">
                  <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-md text-[10px] font-black uppercase">
                    {reservation.table_id} ({reservation.shade})
                  </span>
                </div>
              </div>
            )}
